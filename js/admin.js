	...
	$(window).load(function() {
            ...
            */
            $('.as-slider').find('#as-slider-name').on('keyup change',function() {
                    ...
                    var shortcode = '';
                    shortcode += '[avartanslider alias="';
                    shortcode += alias;
                    shortcode += '"]';
                    var phpfunction = '';
                    phpfunction += 'if(function_exists("avartanslider")) avartanslider("';
                    phpfunction += alias;
                    phpfunction += '");';
                    if(alias != '') {
                    ...
                    update: function(event, ui) {
                            ...
                            $('.as-admin #as-slides .as-slide-tabs ul.as-sortable li').each(function() {
                                    ...
                                    if(!temp.find('a').hasClass('as-add-new')) {
                                            temp.find('a').html(avartanslider_translations.slide + ' <span class="as-slide-index">' + (temp.index() + 1) + '</span><span class="dashicons dashicons-dismiss as-close"></span>');
                                    }
                            });
                    }
            });
            ...
            */
            function  avartansliderAddSlide() {
                    ...
                    add_btn.parent().before('<li class="ui-state-default"><a class="as-button as-is-navy as-is-active">' + avartanslider_translations.slide + ' <span class="as-slide-index">' + (slides_number + 1) + '</span><span class="dashicons dashicons-dismiss as-close"></span></a></li>');
                    ...
                    $('.as-admin #as-slides .as-slides-list .as-slide-settings-list .as-slide-background-type-color-picker-input').wpColorPicker({
                            ...
                            change: function(event, ui){
                                    ...
                                    if(btn.closest('.as-content').find('input[name="as-slide-background-type-color"]:checked').val() == '1') {
                                            ...
                                            area.css('background-color', avartansliderConvertHex(ui.color.toString(),opacity_per));
                                    }
                            },
                            // a callback to fire when the input is emptied or an invalid color
                            clear: function() {},
            ...
            */
            $('.as-admin #as-slides').on('change', '.as-slides-list .as-slide-settings-list input[name="as-slide-background-type-color"]:radio', function() {
                    ...
                    if(btn.val() == '0') {
                            ...
                            var opacity_per = $.trim(btn.closest('.as-content').find('.as-slide-background-opacity').val());
                            area.css('background-color', avartansliderConvertHex(avartansliderRGBToHex(color_picker_value),opacity_per));
                    }
            });
            ...
            function  avartansliderUploadImageElement(slide_parent) {
                    ...
                    file_frame.on( 'select', function() {
                      ...
                      if(! image.is('a')) {
                            ...
                            {
                                avartansliderResizeImage(image);
                            }  
                      }
                      else {
                            image.find('> img').attr('src', image_src);
...
//                            btn.text(avartanslider_translations.slide_live_preview);
           ...
           $('.as-admin #as-slides').on('click', '.as-slide .as-save-slide', function() {
                ...
                avartansliderSaveSlide(slide_id);
            });
                            ...
                            success: function(response) {
                                    ...
                                    if(response !== false && response!='duplicate' && response!='') {
                                        ...
                                        if(response === false || response == '')
                                        {
                                            ...
                                            avartansliderShowMsg(avartanslider_translations.slider_already_find+' "'+ $.trim(content.find('#as-slider-alias').text()) +'" '+avartanslider_translations.slider_exists,'danger');
                                        }
                                    }
                            },
            ...
            */
            function  avartansliderSaveSlide(slide_id) {
