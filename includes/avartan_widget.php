...
class Avartanslider_Widget extends WP_Widget {

    function __construct() {
        $widget_ops = array('classname' => 'widget_avartanslider', 'description' => __('Add slider to your sidebar.', AVARTANSLIDER_TEXTDOMAIN));
        parent::__construct('avartan-slider-widget', __('Avartan Slider Widget', AVARTANSLIDER_TEXTDOMAIN), $widget_ops);
    ...
    function widget($args, $instance) {
        ...
        ob_start();
        extract($args);
        $title = (!empty($instance['title']) ) ? $instance['title'] : __('Avartan Slider', AVARTANSLIDER_TEXTDOMAIN);
    ...
    function form($instance) {
        ...
        ?>
        <p><label for="<?php echo $this->get_field_id('title'); ?>"><?php _e('Title:', AVARTANSLIDER_TEXTDOMAIN); ?></label>
            <input class="widefat" id="<?php echo $this->get_field_id('title'); ?>" name="<?php echo $this->get_field_name('title'); ?>" type="text" value="<?php echo $title; ?>" /></p>
        <p>            
            <?php            
            if ($sliders) {
                ?>
                <label for="<?php echo $this->get_field_id('slider_alias'); ?>"><?php _e('Select Slider:', AVARTANSLIDER_TEXTDOMAIN); ?></label>
                <select class="widefat" id="<?php echo $this->get_field_id('slider_alias'); ?>" name="<?php echo $this->get_field_name('slider_alias'); ?>">                    
                    <option value="0" <?php if ($slider_alias == '0' ) echo 'selected = selected'; ?>><?php _e('select avartan slider',AVARTANSLIDER_TEXTDOMAIN); ?></option>
                    <?php
